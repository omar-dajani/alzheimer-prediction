    ...
    Returns:
        tuple:
            ...
            final_model (GradientBoostingSurvivalAnalysis): Final model fitted
                on all data.
            study (optuna.Study): Completed Optuna study — used downstream for
                optimization history and hyperparameter importance plots.
    """
...
def run_deepsurv(X_imp, y_event, y_duration, label,
    ...
    Features are standardized before training. The final model is refit on a
    stratified 80/20 split (preserving event rate) for early stopping and
    per-epoch loss tracking. Baseline hazards are computed after fitting.
    ...
    Returns:
        tuple:
            ...
            loss_history (dict): {'train': [...], 'val': [...]} lists of per-epoch
                negative partial log-likelihood from the final refit. Used for
                overfitting diagnostics. Empty lists if pycox log unavailable.
            study (optuna.Study): Completed Optuna study — used downstream for
                optimization history and hyperparameter importance plots.
    """
    ...
    # Six architecture options spanning shallow-wide to deep-narrow
    ARCH_OPTIONS = [
        [32, 32],
        [64, 64],
        [128, 128],
    ...
    def objective(trial):
        ...
        for tr, va in skf.split(X_scaled, y_event):
            ...
            optimizer = tt.optim.Adam(lr=lr, weight_decay=wd)
            model = PycoxCoxPH(net, optimizer)
    ...
    optimizer_final = tt.optim.Adam(lr=best['lr'], weight_decay=best['wd'])
    final = PycoxCoxPH(net_final, optimizer_final)
    ...
    log = final.fit(
        X_scaled[idx_tr], y_tr_f,
        best['batch'], 200,
        val_data=(X_scaled[idx_va], y_va_f),
...
def run_cox_ph(X_imp, y_event, y_duration, label, n_trials=30, seed=RANDOM_SEED):
    ...
    def objective(trial):
        # Penalizer floor raised to 0.1 — values below this with any l1 component
        # collapse all coefficients to zero on high-dimensional survival data.
        penalizer = trial.suggest_float('penalizer', 0.1, 10.0, log=True)
        # l1_ratio capped at 0.5 — pure/near-pure lasso zeroes out all coefs,
        # producing identical survival curves and ZeroDivisionError downstream.
            ...
            with warnings.catch_warnings():
                ...
                try:
                    fitter = _fit(X_scaled.iloc[tr], y_event[tr], y_duration[tr],
                                  penalizer, l1_ratio)
                    # Detect collapse — all-zero coefs produce identical predictions
                    # and zero comparable pairs in concordance_td.
